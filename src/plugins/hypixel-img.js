...
const onMessage = async (client, e) => {
    ...
    if (ms[0] == '!hyp' && ms.length == 2) {
        client.sendText(e.channelId, `请使用#hyp`);
    }
    ...
    if (ms[0] == '#hyp') {
        try {
            ...
            if (ping != '200 OK') return client.sendText(e.channelId, '无法连接到后端服务，请联系作者');
            if (ms.length == 1) {
                ...
                client.sendText(e.channelId, res);
            }
            let id = ms[1];
            if (ms.length == 2) {
                let data = await fetch(`${url}/hyp?name=${id}`).then(res => res.json());
                if (data.statusCode == 200 && fs.existsSync(data.data))
                    client.sendImg(e.channelId, data.data);
                else
                    client.sendText(e.channelId, `获取失败：${data.reason}`);
            }
            if (ms.length == 3 || ms.length == 4) {
                ...
                if (data.statusCode == 200 && fs.existsSync(data.data))
                    ...
                    client.sendText(e.channelId, `获取失败：分类不存在，请使用#hyp查看用法`);
                else
                    client.sendText(e.channelId, `获取失败：${data.reason}`);
            }
        } catch (err) {
